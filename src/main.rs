...
fn main() {
    ...
    match app.action {
        ...
        Action::PidStat(pidstat_arg) => {
            let content = std::fs::read_to_string(pidstat_arg.logs_path).unwrap();
            let results = pidstat::parse(&content, std::time::Duration::from_secs(0));

            draw_u64(
                "tmp.png",
                "CPU usage on a  (4C 8G AWS c5.xlarge) machine",
                "timestamp",
                "CPU usage",
                &vec![results
                    .iter()
                    .map(|r| (r.timestamp, r.cpu_cpu_pct as u64))
                    .collect()],
                &["CPU usage".to_string()],
            )
            .unwrap();
        ...
        Action::TimeCost(timecost_arg) => {
            let content = std::fs::read_to_string(timecost_arg.logs_path).unwrap();
            let results = timecost::parse(&content);
            {
                let all_duration = results
                    .iter()
                    .fold(std::time::Duration::default(), |acc, r| {
                        acc + r.full_block_timecost
                    });

                let chain_duration = results
                    .iter()
                    .fold(std::time::Duration::default(), |acc, r| {
                        acc + r.process_block_chain_service
                    });
                println!("all duration: {:?}", all_duration);
                println!("chain duration: {:?}", chain_duration);
                println!(
                    "chain/all: {}",
                    chain_duration.as_millis() as f64 / all_duration.as_millis() as f64
                );
            }
            draw_u64(
                "timecost.png",
                ...
                &vec![results
                    .iter()
                    .map(|r| (r.height, r.full_block_timecost.as_millis() as u64))
                    .collect()],
                &["duration".to_string()],
            )
            .unwrap();
...
impl EntryBlockProcess {
    fn parse_line(&self, line: &str) -> Option<Self> {
        ...
        let time_start = line.find("202")?;

        let time_str = &line[time_start..23].to_string();
        // parse time from string
        let time = chrono::NaiveDateTime::parse_from_str(time_str, "%Y-%m-%d %H:%M:%S%.f").ok()?;
...
fn draw_u64(
    ...
    let root = BitMapBackend::new(filename, (3000, 600)).into_drawing_area();
